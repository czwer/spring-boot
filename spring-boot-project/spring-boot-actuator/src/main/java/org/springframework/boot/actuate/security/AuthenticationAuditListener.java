...
public class AuthenticationAuditListener extends AbstractAuthenticationAuditListener {
	...
	public void onApplicationEvent(AbstractAuthenticationEvent event) {
		if (event instanceof AbstractAuthenticationFailureEvent failureEvent) {
			logger.info("[SPRING_BOOT] 自定义日志---【监听事件】：AbstractAuthenticationEvent(AbstractAuthenticationFailureEvent)，timestamp："+event.getTimestamp());
			...
			logger.info("[SPRING_BOOT] 自定义日志---【监听事件】：AbstractAuthenticationEvent(AuthenticationSuccessEvent)，timestamp："+event.getTimestamp());
			onAuthenticationSuccessEvent(successEvent);
		}
		else if (event instanceof LogoutSuccessEvent logoutSuccessEvent) {
			logger.info("[SPRING_BOOT] 自定义日志---【监听事件】：AbstractAuthenticationEvent(LogoutSuccessEvent)，timestamp："+event.getTimestamp());
